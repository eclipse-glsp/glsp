...
The `@eclipse-glsp/releng` package provides a cli app for assisting with the GLSP release process.
The contributed `glsp-releng` tool, is a command line tool that offers all contributed commands.

## Getting Started

This package is intended for Eclipse GLSP commiters, and is not a general purpose tool.
It's only used internally for creating new releases.
For now, the package is private and not published to npm.
Follow the [instructions](../../docker/releng/README.md) to run the releng cli in docker container.
